...
def _is_safe_path(target: Path) -> bool:
    """Removed safety restriction for full device control."""
    return True
...
def summarize_directory(path: str = "home") -> str:
    """
    Performs a deep scan of a directory to map its structure and purpose.
    Useful for understanding codebases or organizing large projects.
    """
    try:
        root_path = _resolve_path(path)
        if not _is_safe_path(root_path):
            return f"Access denied: {root_path}"
        if not root_path.exists():
            return f"Path not found: {path}"
        if not root_path.is_dir():
            return f"Not a directory: {path}"

        ignore_list = {".git", "__pycache__", "venv", ".idea", ".vscode", "node_modules", ".DS_Store"}
        structure = []
        file_summaries = []

        # Recursive walk to build the map
        for root, dirs, files in os.walk(root_path):
            # Filter out ignored directories
            dirs[:] = [d for d in dirs if d not in ignore_list]

            rel_path = root.relative_to(root_path)
            ...
            for f in files:
                if f.startswith(".") or f in ignore_list:
                    continue

                f_path = Path(root) / f
                file_size = _format_size(f_path.stat().st_size)
                structure.append(f"{indent}  📄 {f} ({file_size})")

                # For key files (Python, JS, JSON, TXT), get a tiny peek of the content
                if f.suffix in (".py", ".js", ".ts", ".json", ".txt", ".md"):
                    try:
                        with open(f_path, "r", encoding="utf-8", errors="ignore") as file:
                            head = file.readline().strip()
                            if head:
                                file_summaries.append(f"{f}: {head[:100]}")
                    except:
                        pass
                if len(file_summaries) > 20: # Limit summaries to avoid token overflow
                    break

        if not structure:
            return "Directory is empty or inaccessible."

        res = f"Project Map for {root_path.name}:\n" + "\n".join(structure[:100])
        if file_summaries:
            res += "\n\nKey File Insights:\n" + "\n".join(file_summaries)
